...
function getStatusButton(status, id , expired) {
    if (status === 'Kerjakan') {
        return `<button type="button" class="btn btn-success btn-small" onclick="showConfirmationModal(${id} , '${expired}')">${status}</button>`;
    } else if (status === 'Waiting') {
        return `<a href="#" class="btn btn-secondary btn-small disabled-link" style="pointer-events: none;">${status}</a>`;
    } else {
        return `<a href="#" class="btn btn-danger btn-small disabled-link" style="pointer-events: none;">${status}</a>`;
    }
}
